...
class PointSource():
    def __init__(self, Mvir, Rvir, theta, phi, vmax, rvmax):
        self.Mvir = Mvir*units.M_s
        self.Rvir = Rvir*units.kpc
        self.vmax = vmax*units.Km/units.Sec
        self.rvmax = rvmax*units.kpc
        self.theta = theta
        self.phi= phi
    ...
    def PSJ_int( self ):
        ...
        self.r_s = self.rvmax / 2.163
        rho_s = 2.163 * self.vmax**2 / (4 * np.pi * G * self.r_s**2 * ( np.log(3.163) - 2.163/3.163 ))
        print(rho_s)

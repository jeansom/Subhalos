...
class PointSource():
    ...
    def PSJ_int( self ):
        ...
        self.c = []
        self.r_s = []
        for iR in range(len(self.Rvir)):
            M_r = 4*np.pi*( np.log( (17*units.kpc + self.Rvir[iR])/(17*units.kpc) ) - self.Rvir[iR]/(17*units.kpc + self.Rvir[iR]) ) * 1.77e12*units.M_s / (4*np.pi*( np.log( (17*units.kpc + 311*units.kpc)/(17*units.kpc) ) - 311*units.kpc/(17*units.kpc + 311*units.kpc) ))
            self.r200.append(( self.Mvir[iR] / ( ( 2 - 1.9 ) * M_r ) )**(1/3) * self.Rvir[iR]) #( 3 * self.Mvir / (4 * np.pi * 200 * rho_c) )**(1./3.)
            self.c.append(self.conc(self.Mvir[iR], self.Rvir[iR], self.r200[iR]))
            self.r_s.append(self.r200[iR]/self.c[iR])

            r_arr = np.linspace(0, self.r200[iR], 500)
            rho0 = self.Mvir[iR] / np.trapz( r_arr**2 * 4 * np.pi * np.exp( (-2./0.16) * ( (r_arr/self.r_s[iR])**(0.16) - 1)), r_arr)
            J_int.append(np.trapz( 4 * np.pi * r_arr**2 * rho0**2 * (np.exp( (-2./0.16) * ( (r_arr/self.r_s[iR])**(0.16) - 1) ) )**2, r_arr ))
        J_int = np.array(J_int)
        self.r200 = np.array(self.r200)

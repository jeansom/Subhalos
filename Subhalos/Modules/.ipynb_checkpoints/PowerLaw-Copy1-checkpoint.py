...
def simple_power(F, fix_first, A, *args):
    results = []
...
def genMinuitString( Nb, A_init, A_bounds, A_error, n_init, n_bounds, n_error, Fb_init, Fb_bounds, Fb_error, fix_nfirst = None, fix_nlast=None, fix_Fbfirst=None, fix_Fblast=None, vmin=-np.inf, vmax=np.inf):
    ...
    return ("minuit = Minuit(lambda %s: -PowerLaw.loglike(AssortedFunctions.myLog(F), AssortedFunctions.myLog(dN/dF), PowerLaw.simple_power, %s, %s, %s, %s), %s)" % (argument_list, str(fix_Fbfirst!=None), argument_list, "vmin="+str(vmin), "vmax="+str(vmax), options));
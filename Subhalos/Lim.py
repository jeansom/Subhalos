...
np.save("MC/subhalo_MC", subhalo_MC)    
...
else: data = makeMockData( subhalo_MC, bkgFac*13.9583217*dif, bkgFac*1.06289421*iso, bkgFac*0.90448092*psc )

if useMadeMC==None: np.save("MC/mockdata_"+str(tag), data)
ll, A, Fb = getNPTFitLL( data, fermi_exposure, mask, Nb, tag, [ [dif, 'dif'], [iso, 'iso'] , [psc, 'psc']], subhalos, *SCD_params )
print(A, Fb)
xsec_test_arr = np.logspace(-40, -20, 101)
ll_arr = []
i_arr, d_arr, p_arr = [], [], []
for xsec_t in xsec_test_arr:
    minuit_min = iminuit.Minuit(lambda d, i, p: -ll([ d, i, p, A/((xsec_t/xsec0)*(PPnoxsec/PPnoxsec0)), *(Fb*((xsec_t/xsec0)*(PPnoxsec/PPnoxsec0))) ]), d=13.9583217, i=1.06289421, p=0.90448092, fix_d=False, fix_i=False, fix_p=False, limit_d=(0.,100.), limit_i=(0.,15.), limit_p=(0.,15.), error_d=1e-1, error_i=1e-1, error_p=1e-1, print_level=1)
    minuit_min.migrad()
...
np.savez("lim_fixed_"+str(xsec_inj) + "_" + str(useData) + "_" + str(Nb) + "_" + tag, [xsec_rec, -minuit_min.fval ], ll_arr, d_arr, i_arr, p_arr )

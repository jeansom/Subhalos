...
for ib in range(len(my_iebins)-1):
    ...
    F_arr_ebins.append(np.histogram(PS_set.J.value[~mask[PS_set.pixels]] * xsec * PPnoxsec, bins=flux_bins)[0])
...
for ib, PS_set in enumerate(PS_arr_ebins):
    ...
    if xsec == 1e-22: np.save("/tigress/somalwar/Subhaloes/Subhalos/MC/FixedSCD/subhalo_flux_map_NFW_"+str(marr[0])+"TeV_"+str(sys.argv[1])+"_"+str(my_iebins[ib])+"-"+str(my_iebins[ib+1])+"_"+str(xsec), flux_map) 
    ...
    scipy_min = minimize( ll, [-6, 10, 0, 0.1, 0.5], method="SLSQP", bounds = [ [-10, -2], [2.05, 15], [-3,3], [-10, 1.95], [0.01, 0.3], [0.01, 0.3] ], options={'ftol':1e-15, 'eps':1e-5, 'maxiter':5000, 'disp':True} )

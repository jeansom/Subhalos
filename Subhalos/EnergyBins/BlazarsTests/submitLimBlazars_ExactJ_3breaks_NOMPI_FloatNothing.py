    ...
    for mass in [ 10, 100, 10000 ]:
        ...
        for ix, xsec in enumerate(xsec_arr):
            for tx, xsectest in enumerate(xsec_test_arr):
                ...
                batchn += "#SBATCH --mem 1GB\n"
                tag = '_FNothing_'+str(mass)+'GeV_'+str((xsec))+'_'+str(t)+"_siginj_"
                batchn += "#SBATCH --output=slurm/slurm"+str(tag)+"_"+str(xsectest)+".out\n"
                batchn += "python LimBlazarsMPI_ExactJ_3breaks_NOMPI_FloatNothing.py -x " + str(xsec) + " -t " + tag +" -r " + str(t) + " -m "+str(mass)+" -c " + str(xsectest) + " \n"

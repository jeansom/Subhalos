...
if comm.rank == 0:
    for ib, b in enumerate(my_iebins[:-1]):
        ...
        data_ebins.append(makeMockData( subhalo_MC[ib], blazars_ebins[ib], bkg_min.x[1]*iso_ebins[ib] ))
...
#    bkg_arr.append([ [ dif_ebins[ib], 'dif'], [iso_ebins[ib], 'iso'], [psc_ebins[ib], 'psc'] ] )
    bkg_arr.append([ [ iso_ebins[ib], 'iso'] ])
...
def ll_func( xsec_t, ix, A_dif, A_iso, A_psc, Ab, n1b, n2b, n3b, Fb1b, Fb2b ): 
    ...
    return -ll_ebins_xsec[ix][ib]([A_iso, Ab, n1b, n2b, n3b, Fb1b, Fb2b, A_ebins_xsec[ix][ib]/((xsec_t/SCD_params_xsec[ix])*PPnoxsec_ebins[ib]/PPnoxsec0_ebins[ib]), *(np.array(Fb_ebins_xsec[ix][ib])*((xsec_t/SCD_params_xsec[ix])*PPnoxsec_ebins[ib]/PPnoxsec0_ebins[ib])) ]) # NON-POISSONIAN BACKGROUNDS
...
/            scipy_min = minimize( lambda args: ll_func( xsec_t, 0, 0, 0, *args ),
            ...
            '''
            ## FLOATING NORM, MID SLOPE
#            scipy_min = minimize( lambda args: ll_func( xsec_t, ix, args[0], 0, 0, args[1], blazar_SCD[1], args[2], blazar_SCD[3], blazar_SCD[4], blazar_SCD[5] ),
#                                  [0.9, -6, 0], method="SLSQP", bounds = [ [0., 10.], [-10, -2], [-3, 3] ], options={'ftol':1e-15, 'eps':1e-10, 'maxiter':5000, 'disp':True} )
            scipy_min = minimize( lambda args: ll_func( xsec_t, ix, 0, 0.14331159, 0, -2-args[1], blazar_SCD[1], -3+args[2], blazar_SCD[3], blazar_SCD[4], blazar_SCD[5] ),
                                  [1e-10, 1e-10, 1e-10], bounds = [ [0., 10.], [0, 8], [0, 6] ], method="L-BFGS-B", options={'maxiter':10000, 'ftol': 1e-10, 'eps':1e-5, 'disp':True} ) 
#            print( ll_func( xsec_t, ix, bkg_min.x[0], 0, 0, *blazar_SCD ) )
#            print(scipy_min.x[0], -2-scipy_min.x[1], -3+scipy_min.x[2])
#            print(bkg_min.x[0], blazar_SCD[0], blazar_SCD[2])
            ll += -scipy_min.fun
            SCDb_arr.append(np.array(scipy_min.x))
            '''

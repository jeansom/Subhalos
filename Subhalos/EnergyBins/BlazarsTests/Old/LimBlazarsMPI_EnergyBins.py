...
parser.add_argument('-e', '--energies', nargs='+', type=int, help='initial energy bins for subhalos')
...
A0 = -5.44404671
n0 = [2.05, 1.76208101, 1.29979701]
Fb = [61.41696489, 0.1]
SCD_params = []
for ib in range(len(my_iebins)-1):
    SCD_params.append(A0)
for ib in range(len(my_iebins)-1):
    SCD_params.extend(n0)
for ib in range(len(my_iebins)-1):
    SCD_params.extend(Fb)
tag = args.tag 
mass = 100
mass_inj = 100
my_iebins0 = np.array(args.energies)
print(my_iebins0)
...
for ib, b in enumerate(my_iebins[:-1]):
    ...
    blazars_ebins.append(np.load("/tigress/somalwar/Subhaloes/Subhalos/EnergyBins/blazarMC/blazar_map_"+str(b)+"_"+str(my_iebins[ib+1])+"_"+str(trial)+".npy")*fermi_exposure)
...
if useSubhalo == None: 
    for ib, b in enumerate(my_iebins[:-1]):
        fake_data = np.load("/tigress/somalwar/Subhaloes/Subhalos/MC/subhalo_flux_map0_"+str(my_iebins0[0])+"-"+str(my_iebins0[1])+".npy")*exposure_ebins[ib]*xsec_inj/xsec0*PPnoxsec_ebins[ib]/PPnoxsec0
        fake_data = np.round(fake_data).astype(np.int32)
        subhalo_MC.append(fake_data)
else: 
    for ib, b in enumerate(my_iebins[:-1]):
        subhalo_MC.append(np.load(useSubhalo+str(my_iebins0[0])+"-"+str(my_iebins0[1])+".npy")*exposure_ebins[ib]*xsec_inj/xsec0*PPnoxsec_ebins[ib]/PPnoxsec0)
print(PPnoxsec_ebins[ib]/PPnoxsec0)
...
#        data_ebins.append(makeMockData( subhalo_MC[ib], blazars_ebins[ib], bkg_min.x[0]*dif_ebins[ib], bkg_min.x[1]*iso_ebins[ib], bkg_min.x[2]*psc_ebins[ib] ))
        data_ebins.append(makeMockData( subhalo_MC[ib], bkg_min.x[0]*dif_ebins[ib], bkg_min.x[1]*iso_ebins[ib], bkg_min.x[2]*psc_ebins[ib] ))
...
ll_arr = []
SCDb_arr_ebins = []
def ll_func( xsec_t, ib, A_dif, A_iso, A_psc, Ab, n1b, n2b, n3b, Fb1b, Fb2b ): 
#    return -ll_ebins[ib]([A_dif, A_iso, A_psc, Ab, n1b, n2b, n3b, Fb1b, Fb2b, A_ebins[ib]/((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0), *(np.array(Fb_ebins[ib])*((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0)) ]) # ALL BACKGROUNDS                                                  
 
#    return -ll_ebins[ib]([Ab, n1b, n2b, n3b, Fb1b, Fb2b, A_ebins[ib]/((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0), *(np.array(Fb_ebins[ib])*((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0)) ]) # NON-POISSONIAN BACKGROUNDS

    return -ll_ebins[ib]([A_dif, A_iso, A_psc, A_ebins[ib]/((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0), *(np.array(Fb_ebins[ib])*((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0)) ]) # POISSON BACKGROUNDS                                                                              

#    return -ll_ebins[ib]([A_ebins[ib]/((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0), *(np.array(Fb_ebins[ib])*((xsec_t/xsec0)*(PPnoxsec_ebins[ib]/PPnoxsec0))) ]) # NO BACKGROUNDS
...
for xsec_t in my_xsec_test_arr:
    ...
    for ib in range(len(my_iebins)-1):
        ...
        if PPnoxsec_ebins[ib] != 0:
            '''
            ## ALL BACKGROUNDS
            scipy_min = minimize( lambda args: ll_func( xsec_t, ib, *args ),
            ...
            '''
            '''
            ## NON-POISSONIAN BACKGROUNDS
            scipy_min = minimize( lambda args: ll_func( xsec_t, ib, 0., 0., 0., *args ),
            ...
            ## POISSON BACKGROUNDS
            scipy_min = minimize( lambda args: ll_func( xsec_t, ib, *args, 0., 1, 1., 0.1, 1000, 0.5 ),
                                  [0.1, 0.1, 0.1], method="SLSQP", bounds = [ [0, 20], [0, 20], [0, 20] ], options={'ftol':1e-15, 'eps':1e-5, 'maxiter':5000, 'disp':True} )
            ll += -scipy_min.fun
            SCDb_arr.append(np.array(scipy_min.x))
            ...
            '''
            ## NO BACKGROUNDS
            ll += -ll_func( xsec_t,ib, 0.,0.,0., 0., 1, 1., 0.1, 1000000., 0.5 )

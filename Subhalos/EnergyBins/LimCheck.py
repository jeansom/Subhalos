...
if useData: 
    ...
    for ib, b in enumerate(my_iebins[:-1]):
        ...
        data_ebins.append(makeMockData( subhalo_MC[ib], bkgFac*best_fit_params['d']*dif_ebins[ib], bkgFac*best_fit_params['i']*iso_ebins[ib], bkgFac*best_fit_params['p']*psc_ebins[ib] ))
...
ll_arr = []
i_arr_ebins, d_arr_ebins, p_arr_ebins = [], [], []
for xsec_t in xsec_test_arr:
    ll = 0
    for ib in range(len(my_iebins[:-1])):
        i_arr, d_arr, p_arr = [], [], []
        if PPnoxsec_ebins[ib] != 0:
            minuit_min = iminuit.Minuit(lambda d, i, p: -ll_ebins[ib]([d, i, p, A_ebins[ib]/((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0_ebins[ib]), *(np.array(Fb_ebins[ib])*((xsec_t/xsec0)*PPnoxsec_ebins[ib]/PPnoxsec0_ebins[ib])) ]), d= best_fit_params['d'], i =   best_fit_params['i'], p =  best_fit_params['p'], fix_d=True, fix_i=True, fix_p=True, limit_d=(0.,100.), limit_i=(0.,15.), limit_p=(0.,15.), error_d=1e-1, error_i=1e-1, error_p=1e-1, print_level=1)
            minuit_min.migrad()
            ll += (-minuit_min.fval)
...
for xi in range(max_loc, len(xsec_test_arr)):
    ...
    if val < -2.71:
        ...
        break
np.savez("lim/Check/lim_fixed_"+str(xsec_inj) + "_" + str(useData) + "_" + str(Nb) + "_" + tag, [xsec_rec, -minuit_min.fval ], ll_arr, d_arr_ebins, i_arr_ebins, p_arr_ebins )

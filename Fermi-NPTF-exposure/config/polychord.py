...
class Analyzer(object):
	...
	def __init__(self, n_params, outputfiles_basename = "chains/1-"):
		...
		self.data_file = self.outputfiles_basename + 'test_equal_weights.txt'
		...
		self.post_file = self.data_file
		#"%spost_seperate.dat" % self.outputfiles_basename + test_equal_weights.txt
		...
		"""
		self.stats_file = self.outputfiles_basename + 'test.stats'
	...
	def get_mode_stats(self):
		...
		self._read_error_into_dict(lines[8], stats)
		if 'log(Z)'.lower() in stats:
			stats['global evidence'] = stats['log(Z)'.lower()]
			stats['global evidence error'] = stats['log(Z) error'.lower()]

			stats['nested sampling global log-evidence'] = stats['global evidence']
			stats['nested sampling global log-evidence error'] = stats['global evidence error']

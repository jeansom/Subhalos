...
def fits_filename(CTB_dir, en_bin, nopsc, smoothed, is_p8, is_ps_model=False, is_ps_mask=False, eventclass=5,
                  eventtype=3):
...
def fits_filename_newstyle(CTB_dir, en_bin, nopsc, smoothed, is_p8, is_ps_model=False, is_ps_mask=False, eventclass=5,
                           eventtype=3):
    # function to help build .fits filenames depending on energy bin, point-source subtraction, and smoothing
    # First establish which directory - depends on eventclass. Only setup for ultracleanveto (5) and source (2) at the moment
    if eventclass == 2:
        subdir = 'PASS8_July15_Source/specbin/'
    if eventclass == 5:
        subdir = 'PASS8_July15_UltracleanVeto/specbin/'
    if eventclass == 4:
        subdir = 'PASS8_July15_Ultraclean/specbin/'
...
def get_CTB(CTB_dir, nside, CTB_en_min=0, CTB_en_max=num_en_bins, is_p8=False, eventclass=5, eventtype=3, newstyle=0):
    ...
    if CTB_en_min > 0:
        if newstyle:
            map_nopsc_filename = fits_filename_newstyle(CTB_dir, CTB_en_min - 1, nopsc=True, smoothed=False,
                                                        is_p8=is_p8, eventclass=eventclass, eventtype=eventtype)
        else:
            map_nopsc_filename = fits_filename(CTB_dir, CTB_en_min - 1, nopsc=True, smoothed=False, is_p8=is_p8,
    ...
    for en_bin in range(CTB_en_min, CTB_en_max):
        if newstyle:
            map_nopsc_filename = fits_filename_newstyle(CTB_dir, en_bin, nopsc=True, smoothed=False, is_p8=is_p8,
                                                        eventclass=eventclass, eventtype=eventtype)
            map_psc_filename = fits_filename_newstyle(CTB_dir, en_bin, nopsc=False, smoothed=False, is_p8=is_p8,
                                                      eventclass=eventclass, eventtype=eventtype)
        else:
            map_nopsc_filename = fits_filename(CTB_dir, en_bin, nopsc=True, smoothed=False, is_p8=is_p8,
                                               eventclass=eventclass, eventtype=eventtype)
            map_psc_filename = fits_filename(CTB_dir, en_bin, nopsc=False, smoothed=False, is_p8=is_p8,
                                             eventclass=eventclass, eventtype=eventtype)
        map_nopsc = fits.open(map_nopsc_filename)
...
def get_ps_models(CTB_dir, nside, CTB_en_min=0, CTB_en_max=num_en_bins, eventclass=5, eventtype=3, newstyle=0):
    ps_models = []
    for en_bin in range(CTB_en_min, CTB_en_max):
        if newstyle:
            f = fits.open(
                fits_filename_newstyle(CTB_dir, en_bin, 0, 0, is_p8=True, is_ps_model=True, eventclass=eventclass,
                                       eventtype=eventtype))
            f_mask = fits.open(
                fits_filename_newstyle(CTB_dir, en_bin, 0, 0, is_p8=True, eventclass=eventclass, eventtype=eventtype))
        else:
            f = fits.open(fits_filename(CTB_dir, en_bin, 0, 0, is_p8=True, is_ps_model=True, eventclass=eventclass,
                                        eventtype=eventtype))
            f_mask = fits.open(
                fits_filename(CTB_dir, en_bin, 0, 0, is_p8=True, eventclass=eventclass, eventtype=eventtype))
...
def get_ps_masks(CTB_dir, nside, CTB_en_min=0, CTB_en_max=num_en_bins, eventclass=5, eventtype=3, newstyle=0):
    ps_masks = []
    for en_bin in range(CTB_en_min, CTB_en_max):
        if newstyle:
            f = fits.open(
                fits_filename_newstyle(CTB_dir, en_bin, 0, 0, is_p8=True, is_ps_mask=True, eventclass=eventclass,
                                       eventtype=eventtype))
        else:
            f = fits.open(fits_filename(CTB_dir, en_bin, 0, 0, is_p8=True, is_ps_mask=True, eventclass=eventclass,
                                        eventtype=eventtype))
...
def get_CTB_total_count_maps(CTB_dir, nside, min_en_bin=0, max_en_bin=num_en_bins, is_p8=False, eventclass=5,
    ...
    for en_bin in range(min_en_bin, max_en_bin):
        if newstyle:
            total_map += read_CTB_count_map(
                fits_filename_newstyle(CTB_dir, en_bin, nopsc=False, smoothed=False, is_p8=is_p8, eventclass=eventclass,
                                       eventtype=eventtype), nside)
            total_map_nopsc += read_CTB_count_map(
                fits_filename_newstyle(CTB_dir, en_bin, nopsc=True, smoothed=False, is_p8=is_p8, eventclass=eventclass,
                                       eventtype=eventtype), nside)
            total_map_smth += read_CTB_count_map(
                fits_filename_newstyle(CTB_dir, en_bin, nopsc=False, smoothed=True, is_p8=is_p8, eventclass=eventclass,
                                       eventtype=eventtype), nside)
            total_map_nopsc_smth += read_CTB_count_map(
                fits_filename_newstyle(CTB_dir, en_bin, nopsc=True, smoothed=True, is_p8=is_p8, eventclass=eventclass,
                                       eventtype=eventtype), nside)
        else:
            total_map += read_CTB_count_map(
                fits_filename(CTB_dir, en_bin, nopsc=False, smoothed=False, is_p8=is_p8, eventclass=eventclass,
                              eventtype=eventtype), nside)
            total_map_nopsc += read_CTB_count_map(
                fits_filename(CTB_dir, en_bin, nopsc=True, smoothed=False, is_p8=is_p8, eventclass=eventclass,
                              eventtype=eventtype), nside)
            total_map_smth += read_CTB_count_map(
                fits_filename(CTB_dir, en_bin, nopsc=False, smoothed=True, is_p8=is_p8, eventclass=eventclass,
                              eventtype=eventtype), nside)
            total_map_nopsc_smth += read_CTB_count_map(
                fits_filename(CTB_dir, en_bin, nopsc=True, smoothed=True, is_p8=is_p8, eventclass=eventclass,
                              eventtype=eventtype), nside)

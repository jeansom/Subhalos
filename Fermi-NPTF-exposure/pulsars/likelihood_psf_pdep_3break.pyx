...
def log_nu_k_ary_PSF_exact_1_PS_3break(double[::1] xbg_PSF_compressed, double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, double[::1] PS_dist_compressed, int[::1] data):
    return log_nu_k_ary_PSF_exact_1_PS_3break_int(xbg_PSF_compressed,theta, f_ary, df_rho_div_f_ary, PS_dist_compressed, data)
...
cdef double log_nu_k_ary_PSF_exact_1_PS_3break_int(double[::1] xbg_PSF_compressed, double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, double[::1] PS_dist_compressed, int[::1] data):
    ...
    cdef double n4 = np.float(theta[4])
    cdef double Sb1 = np.float(theta[5])
    cdef double Sb2 = np.float(theta[6])
    cdef double Sb3 = np.float(theta[7])
    ...
    cdef double[::1] g3_ary_f = np.zeros(k_max + 1, dtype=DTYPE)
    ...
    for f_index in range(len(f_ary)):
        ...
        g0_ary_f = spc.gammainc_up_fct_ary_log(k_max, 1. - n1, Sb3 * f2) 
        g1_ary_f = spc.gammainc_up_fct_ary_log(k_max, 1. - n2, Sb2 * f2) - spc.gammainc_up_fct_ary_log(k_max, 1. - n2, Sb3 * f2)
        g2_ary_f = spc.gammainc_up_fct_ary_log(k_max, 1. - n3, Sb1 * f2) - spc.gammainc_up_fct_ary_log(k_max, 1. - n3, Sb2 * f2)
        g3_ary_f = spc.gammainc_lo_fct_ary_back(k_max, 1. - n4, Sb1 * f2)

        kappa = pow(Sb2 / Sb1,1-n3)
        kappa2 = kappa * pow(Sb3 / Sb2,1-n2) 
        term1 = (A * Sb1 * f2) \
                             * (kappa/(n2-1.)+1./(n3-1.) + 1./(1.-n4) - pow(Sb1 / Sb2, n3-1.)/(n3-1.) - kappa * pow(Sb2 / Sb3, n2-1.)/(n2-1.) \
                                - (kappa*pow(Sb2 * f2, n2-1.) * g1_ary_f[0]+pow(Sb1 * f2, n3-1.) * g2_ary_f[0] + pow(Sb1 * f2, n4-1.) * g3_ary_f[0])   + kappa2*( 1./(n1-1.) - pow(Sb3*f2, n1 - 1.)*g0_ary_f[0] )  )
        second_1_alpha = A * pow(Sb3 * f2, n1)* pow(Sb1 / Sb2 , n3)* pow(Sb2 / Sb3 , n2)
        second_1_a =  A * pow(Sb2 * f2, n2)* pow(Sb1 / Sb2 , n3)
        second_1_b = A * pow(Sb1 * f2, n3)
        second_1_bp = A * pow(Sb1 * f2, n4)
        ...
        for p in range(npixROI):
            ...
            second_1_dp = second_1_bp * PS_dist_compressed[p]
            for k in range(data[p]+1):
                x_m_ary_f = second_1_beta * g0_ary_f[k] + second_1_c  * g1_ary_f[k] + second_1_d * g2_ary_f[k]+ second_1_dp * g3_ary_f[k] 
